...
bool Exporter::exportToJS(ChartProject* chartProject, string toFile) {
	...
	writeStringToFile(saveFile, "\tcalcInputs=()=>{\n\t\tconst alerts=[];\n\t\tfor (let obj of this.calcArray){\n\t\t\tif (obj.type==='input') obj.calc(this.calcArray, alerts);\n\t\t}\n\t\treturn alerts;\n\t};\n\n");
	writeStringToFile(saveFile, "\tcalc=()=>{\n\t\tconst alerts=[];\n\t\tfor (let obj of this.calcArray){\n\t\t\tobj.calc(this.calcArray, alerts);\n\t\t}\n\t\treturn alerts;\n\t};\n}");
	fclose(saveFile);
	return true;
}

bool Exporter::exportToJSON(ChartProject* chartProject, string toFile) {
	if (chartProject == nullptr) return false;
	...
	for (auto & constant : chartProject->constants) {
		writeStringToFile(saveFile, "    {\n");
		writeStringToFile(saveFile, "        'type': 'constant',\n");
		writeStringToFile(saveFile, "        'name': '" + constant->getName() + "',\n");
		writeStringToFile(saveFile, "        'value': ");
		writeStringToFile(saveFile, constant->result ? trimmedDoubleToString(*constant->result) : "0");
		writeStringToFile(saveFile, "\n    },\n");
	}
	...
	for (auto & input : chartProject->inputs) {
		writeStringToFile(saveFile, "    {\n");
		writeStringToFile(saveFile, "        'type': 'input',\n");
		writeStringToFile(saveFile, "        'name': '"+ input->getName()+"',\n");
		writeStringToFile(saveFile, "        'code': '");
		writeCode(saveFile, input->getCode());
		writeStringToFile(saveFile, "',\n");
		writeStringToFile(saveFile, "        'input': '");
		writeCode(saveFile, input->input);
		writeStringToFile(saveFile, "',\n");
		writeStringToFile(saveFile, "    },\n");
	}
		...
		case ChartObjectType::ClampChart:
			{
				...
				if (clamp) {
					writeStringToFile(saveFile, "    {\n");
					writeStringToFile(saveFile, "        'type': 'clamp',\n");
					writeStringToFile(saveFile, "        'name': '" + clamp->getName() + "',\n");
					writeStringToFile(saveFile, "        'yIsInputAxis': "+(clamp->yIsInputAxis ? string("true,\n") : string("false,\n")));
					writeStringToFile(saveFile, getJSName(clamp->inputVariable1, true));
					writeStringToFile(saveFile, ",");
					writeStringToFile(saveFile, getJSName(clamp->inputVariable2, true));
					writeStringToFile(saveFile, ",");
